...
async function fetchMyReviews(page, limit) {
    try {
        const response = await fetch(`${API}/admin/get-all-reviews?page=${page}&limit=${limit}`, {
            method: 'GET',
            headers: {
...
async function submitResponse(reviewId) {
    ...
    if (review.Response) return alert("Staff has already responded to this review.");
    ...
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${API}/admin/respond-to-review`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
