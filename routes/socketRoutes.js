...
const { staffLogin, getMyReviewsById, respondToReview, getAppointmentsByStaff,
    changeAvailabilityByStaff
 } = require("../controllers/staff");
const { addService, getAllServices, updateService, addWorkingHours, getWorkingHours,
    addStaff, getAllStaff, editStaff, deleteStaff, getAllAppointments, updateWorkingHours,
    getAppointmentById, updateAppointment, getStaffById, getAllReviewsForAdmin,
respondToReviewByAdmin } = require("../controllers/admin");
const { getAvailableSlots, bookAppointment,
    getAppointments, rescheduleAppointment, cancelAppointment, checkAppointmentStatus,
    getProfile, updateProfile, addReview, getMyReviews, getStaffByService,
...
module.exports = (io, socket) => {
    ...
    socket.use(async ([event, ...args], next) => {
        ...
        require("../utils/auth").authenticateSocket(socket, async (err) => {
            if (err) {
    ...
    socket.on('get-staff', (data) => getAllStaff(io, socket, data));
    ...
    socket.on('update-working-hours', (data) => updateWorkingHours(io, socket, data));
    socket.on('get-staff-by-service', (data) => getStaffByService(io, socket, data));
    ...
    socket.on('get-my-reviews', (data) => {
        getMyReviews(io, socket, data);
    });
    ...
    socket.on('get-staff-by-id', (data) => {
        getStaffById(io, socket, data);
    });
    socket.on('get-my-reviews-by-id', (data) => {
        getMyReviewsById(io, socket, data);
    });
    socket.on('get-my-appointments', (data) => getAppointmentsByStaff(io, socket, data));
    socket.on('change-availability', (data) => changeAvailabilityByStaff(io, socket, data));
    socket.on('get-all-reviews', () => getAllReviewsForAdmin(io, socket));
    socket.on("respond-review-by-admin", (data) => respondToReviewByAdmin(io, socket, data));
